...
class DBConnectionPool():
    """ DBConnectionPool class for managing PostgreSQL connections.
    """
    ...
    def connect(self, timeout: float):
        ...
        if self.timeout > 60:
            raise ValueError(
                "Timeout must be less than or equal to 60 seconds"
            )
        if self.min_size < 1:
            raise ValueError(
                "Minimum pool size must be greater than or equal to 1"
            )
